...
def show_loss_acc(history):
    ...
    plt.xlabel('epoch')
    plt.savefig('results_mobilenet_epoch200.png', dpi=100)


# 训练模型主流程
def train(epochs):
    begin_time = time()  # 记录开始时间
    train_ds, val_ds, class_names = data_load("C:\Project\dataset2\leaf", 224, 224, 16)  # todo 记录数据集位置，修改为自己的数据集位置
    ...
    model.save("models/mobilenet_03.h5")  # todo 修改了自己的模型保存位置，保存模型
